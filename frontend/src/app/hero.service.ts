...
export class HeroService {
    ...
    //http://localhost/imagefootage/backend/api/ For Local
    private heroesUrl = 'https://imagefootage.com/backend/api/';  // URL to web api
    private localhostUrl = 'http://localhost/imagefootage/backend/api/';
	...
	getRelatedProductData(searchData: any,imgtype: any,pagenumber:any): Observable<any> {
        let url = `${this.heroesUrl}relatedsearch`;
        return this.http.post<any>(url, {searchData,imgtype,pagenumber}, this.httpOptions).pipe(
            map(searchResultSet => {
                return searchResultSet;
            }),
            catchError(this.handleError<any>(`unable to get data`))
    ...
    removeDataFromWishlist(product:any): Observable<any> {
    ...
    addWishListItemsData (product:any): Observable<any> {
    ...
    download(product: any): Observable<userData> {
    ...
    downloadindi(product: any): Observable<userData> {
        let tokenData = JSON.parse(product.token);
        const url = `${this.heroesUrl}downloadindi`;
        // var options = {
        //     headers: new HttpHeaders()
        //         .set('Authorization',  `Bearer ${tokenData.access_token}`)
        //         .set('Content-Type',  `application/json`)
