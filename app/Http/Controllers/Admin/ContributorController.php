...
class ContributorController extends Controller
{
	...
	public function ajaxRequestForUserPass($id)
	{
		...
		if (!empty($cemail) && filter_var($cemail, FILTER_VALIDATE_EMAIL)) {
			$token = str_random(64);
			$findUser = DB::table('password_resets')->where('email', $cemail)->limit(1)->first();
			if ($findUser) {
				DB::table('password_resets')->where('email', $cemail)->update([
					'token' => $token,
			...
			$urltoken = base64_encode(json_encode(['email' => $cemail, 'token' => $token]));
			Log::info('Password reset token generated: ' . $urltoken);
			$url = config('app.front_end_url') . "resetpassword/" . $urltoken;
			$data = array('url' => $url, 'email' => $cemail);
			try {
				Mail::send('email.forgotpasswordadmin', $data, function ($message) use ($data) {
					...
					$resp['statusdesc'] = "Check your email for reset password link.";
					$resp['statuscode'] = "1";
			...
			$user = User::where('email', $cemail)->first();
			$user_data = ['user_id' => $id, 'email' => $user->email, 'mobile' => $user->mobile];
			$result = ['status' => 1, 'message' => 'Check your email for reset password link.', 'data' => $user_data];
		} else {
			$resp['statusdesc']  =   "Invalid email address.";
			$resp['statuscode']   =   "0";
			return response()->json(compact('resp'));
		}
	}
