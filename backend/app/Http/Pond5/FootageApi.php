...
class FootageApi {
    ...
    public function  __construct(){
            ...
            if ($envoirement=='local') {
                ...
                $this->api_key =  'FhuBLaPC288';
                $this->api_secret = '0n7ZNva6be9o288';
    ...
    public function download_old($data,$id){
    ...
    public function download($data, $id, $tier){
        ini_set('max_execution_time',0);
        $keys = ['1' => array('key:FhuBLaPC288','secret:0n7ZNva6be9o288'), '2' => array('key:6lR6VMzo289','secret:13Gj1Pm6M631289'), '3' => array('key:1d1tr3hV290','secret:aXCrjlrQZ44K290')];
        $url = $this->url.'/api/v2/download/'.$data['version'].'/'.$id;
        $curl = curl_init();
            //Set some options - we are passing in a useragent too here
        curl_setopt_array($curl, [
                CURLOPT_RETURNTRANSFER => 1,
                CURLOPT_URL => $url,
                CURLOPT_HTTPHEADER => $keys[$tier]
        ]);
            // Send the request & save response to $resp
        $response = curl_exec($curl);
        curl_close($curl);
    ...
    public function getclipdata_latest_version($footage_id){
        $keys = ['commercial' => array('key:FhuBLaPC288','secret:0n7ZNva6be9o288'), 'non-commercial' => array('key:6lR6VMzo289','secret:13Gj1Pm6M631289'), 'web' => array('key:1d1tr3hV290','secret:aXCrjlrQZ44K290')];
        $data = [];
        foreach($keys as $key=>$eachvalue){
            $url = $this->url.'/api/v2/items/'.$footage_id; 
            $curl = curl_init();
            //Set some options - we are passing in a useragent too here
            curl_setopt_array($curl, [
                CURLOPT_RETURNTRANSFER => 1,
                CURLOPT_URL => $url,
                CURLOPT_HTTPHEADER => $eachvalue
            ]);
            // Send the request & save response to $resp
            $response = curl_exec($curl);
            curl_close($curl);
